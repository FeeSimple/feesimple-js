...
const { 
  ...
  timestamp2String, mapFloorplanIdToName
} = require('./helper');

const feeSimpleChainConf = {
  chainId: '1c6ae7719a2a3b4ecb19584a30ff510ba1b6ded86e1fd8b8fc22f1179c622a32',
  // httpEndpoint: 'http://feesimpletracker.io:8877', // for running "test/mytest.js"
  httpEndpoint: 'https://feesimpletracker.io:8878',
  expireInSeconds: 60,
  broadcast: true,
  verbose: false,
  sign: true
...
FeeSimpleJs.prototype = {
  ...
  getAvailabilityData: async function (accountName) {
    ...
    const floorplanImgList = await this.getFloorplanImgList(accountName);
    const propertyToFloorplanList = mapPropertyToFloorplanList(floorplanList, floorplanImgList);
    const floorplanIdToName = mapFloorplanIdToName(floorplanList);
    
    const unitList = await this.getUnitList(accountName);
    const unitImgList = await this.getUnitImgList(accountName);
    const propertyToUnitList = mapPropertyToUnitList(unitList, unitImgList, floorplanIdToName);
    ...
    for (let i=0; i<propertyList.length; i++) {
      ...
      returnedProperty.created_at = timestamp2String(propertyList[i].created_at);
