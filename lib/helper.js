...
const mapFloorplanIdToName = (floorplanList) => {
  let returnedMap = {};

  for (let i=0; i<floorplanList.length; i++) {
    returnedMap[floorplanList[i].id] = floorplanList[i].name;
  }
...
const mapPropertyToUnitList = (unitList, unitImgList, floorplanIdToName) => {
  let returnedMap = {};
  ...
  for (let i=0; i<unitList.length; i++) {
    ...
    if (floorplanIdToName && Object.keys(floorplanIdToName).length > 0) {
      returnedUnit.floor_plan = floorplanIdToName[returnedUnit.floor_plan] || 'None'
    } else {
      returnedUnit.floor_plan = 'None'
    }
